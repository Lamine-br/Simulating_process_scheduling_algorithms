...
let files = new File_Multiniveaux();
...
function Ordonnanceur_FMAR(){
    if (scheduler.getFiles().getFiles().length != 1){
        stop = false ;
        ...
        alert("Il faut initialiser les Files !") ;
    }
}
...
function Ord_FMAR(temps){
    ...
    if (scheduler.getDispatcher().getSignal() === true) {
        num_file = scheduler.getFiles().Ordonnanceur_FMAR();
        if (num_file !== -1) {
        ...
        if (scheduler.getProcesseur().ProcesseurActif()) {
            if (scheduler.getProcesseur().getProcessus().getTempsRestant() === 0) {
                ...
                if (cpt === quantum) {
                    if (num_file < scheduler.getFiles().getFiles().length - 1) {
                        console.log('t = ' + t + ' : ' + 'Désactivation du processus' + scheduler.getProcesseur().getProcessus().getPCB().getPID());
                        scheduler.getFiles().Affichage_file(num_file + 1) ;
                        finish() ;
                        scheduler.DesactiverProcessus(num_file + 1 , temps);
                        console.log(num_file + 1) ;
                        console.log(scheduler.getFiles()) ;
                    } else {
                        console.log('t = ' + t + ' : ' + 'Désactivation du processus' + scheduler.getProcesseur().getProcessus().getPCB().getPID());
                        scheduler.getFiles().Affichage_file(num_file) ;
    ...
    if (!arret) {
        for (let j = 0; j < scheduler.getFileBloquee().getFile().length; j++) {
            if (scheduler.getFileBloquee().getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                ...
                scheduler.getFiles().Affichage_file(scheduler.getFileBloquee().getFile()[j].getPriorite()) ;
                scheduler.ReveillerProcessus(j, scheduler.getFileBloquee().getFile()[j].getPriorite() , temps);
                arret = true;
            }
        }
        if (!arret) {
...
let inpu = document.getElementById('NbFiles' ) ;
inpu.addEventListener('input' , function(e){
    let select = document.getElementById('choix_nf') ;
        let div = document.getElementById('inputs1');
        if(select !=undefined){
            div.removeChild(select) ;
            div.removeChild(document.getElementById('Quantum')) ;
        }
    if(parseInt(e.target.value , 10) > 0){
        for(let i = 0 ; i < parseInt(e.target.value , 10) ; i++)
        {
            scheduler.getFiles().setFile(i , new File_Attente()) ;
        }
        let select  = document.createElement('select') ;
        select.id = "choix_nf" ;
        for(let i=0 ; i<parseInt(e.target.value , 10) ;i++){
            let option = document.createElement('option') ;
            option.textContent = "File " + (i +1) ;
            select.appendChild(option) ;
            select.classList.add('form-control') ;
        }
        let div = document.getElementById('inputs1');
        ...
        inp1.addEventListener('input' , function(e){
            if(parseInt(e.target.value , 10) > 0){
                scheduler.getFiles().setFile(select.selectedIndex,new File_Attente(0 , parseInt(e.target.value , 10)))
            }
        })


        select.addEventListener("change" , function(){
            let i1 = document.getElementById('Quantum') ;
            if(scheduler.getFiles().getFile(select.selectedIndex).getQuantum() != undefined){
                i1.value =  scheduler.getFiles().getFile(select.selectedIndex).getQuantum() ;
            }else{
                i1.value = "" ;
            }
            i1.placeholder = 'Quantum' ;
...
function Creer(){
    ...
    process.setInterruptions([]) ;
    if(document.getElementById("choix_nf") != undefined){
        let sel = document.getElementById("choix_nf") ;
        sel.parentElement.removeChild(sel) ;
        let inp3 = document.getElementById("Quantum") ;
        inp3.parentElement.removeChild(inp3) ;
    }
    let temps = parseInt(document.getElementsByClassName('vit')[0].value , 10) ;
    creation = true ;
    i++ ; 
    ...
    scheduler.CreerProcessus(processus,0,temps) ;   
