...
class Scheduler {
  ...
  DetruireProcessus(temps) {
   ...
   /**************************Ordonnanceur fifo utilisé pour la creation des tables d'affichage************** */
   Ordonnanceur_FIFO(){
    let i = this.#processus.length , j = 0 , t = 0 , h = 0 ,somme_at=0,somme_res=0,nbr_rep=0,nbr_res=0,nbr_at=0, somme_rep=0,ut=0,taux=0, arret = false , cpt = 0 , num_file = 0 ;
    while(j !== i){
        ...
        if(!arret){
            if(this.#processeur.ProcesseurActif()){
                ...
                console.log("===================le temps d'utilsation est:==========="+this.#processeur.getTempsUtilisation());
...
/***********************************Ordonnanceur FIFO (1)***************************************************/
Ordonnanceur_FIFO1() /* POUR CONSTRUIRE LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST ACTIF */ {
  let i = this.#processus.length, /* LE NOMBRE DES PROCESSUS */
    j = 0,
    t = 0,
    h = 0,
    arret = false,
    ...
    num_file = 0; 
  /*-----------------------------------------------------------------------------
  ON VA CREER UNE TABLE DE TAILLE (i = nombre de processus) DONT CHAQUE CASE 
  CONTIENT UN TABLEAU DYNAMIQUE DES DIFFERENTS INSTANTS DE L'ACTIVATION, 
  LA DISACTIVATION, LE BLOQUAGE ET ENFIN LA DESTRUCTION DE CHAQUE PROCESSUS 
  AVEC ORDRE POUR CONSTRUIRE LES INTERVALLES DANS LESQUELS LE PROCESSUS EST 
  EN ETAT ACTIF 
  -----------------------------------------------------------------------------*/
  var TableauFIFO1 = Array(i);
  ...
  while (j !== i) {
    ...
    if (this.#dispatcher.getSignal() === true) {
      if (this.#files.getFile(0).getFile().length !== 0) {
        this.ActiverProcessus(num_file);
        /*----------------------------------------------------------------------
        LORSQUE LE PROCESSUS S'ACTIVE ON ENREGISTRE L'INSTANT D'ACTIVATION 
        DANS LE TABLEAU ASSOCIE A CE PROCESSUS
        ----------------------------------------------------------------------*/
        TableauFIFO1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
        /*----------------------------------------------------------------------*/
        this.#dispatcher.IncrementerNb();
        console.log(
          "t = " +
      ...
      if (this.#processeur.ProcesseurActif()) {
        if (this.#processeur.getProcessus().getTempsRestant() === 0) {
          /*----------------------------------------------------------------------
          LORSQUE LE PROCESSUS SE DETRUIT ON ENREGISTRE L'INSTANT DE SA DESTRUCTION 
          DANS LE TABLEAU ASSOCIE A CE PROCESSUS
          ----------------------------------------------------------------------*/
          TableauFIFO1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          ...
          if (
            ...
            if (
              ...
              this.#processeur.getProcessus().setPriorite(num_file);
              /*----------------------------------------------------------------------
              LORSQUE LE PROCESSUS SE BLOQUE SUITE A UNE INTERRUPTION ON ENREGISTRE 
              L'INSTANT DU BLOQUAGE DANS LE TABLEAU ASSOCIE A CE PROCESSUS
              ----------------------------------------------------------------------*/
              TableauFIFO1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
              /*----------------------------------------------------------------------*/
              console.log(
  ...
  /*---------------------------------------------------------------*/
  return TableauFIFO1; /* ON RETOURNE LE TABLEAU QUI CONTIENT LES DIFFERENTS INSTANTS */
  /*---------------------------------------------------------------*/
  // console.log("\n--------Fin de l'éxecution---------");
}
/***********************************Ordonnanceur FIFO (2)***************************************************/
Ordonnanceur_FIFO2() /* POUR CONSTRUIRE LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST BLOQUE */{
  ...
  /*-----------------------------------------------------------------------------
  ON VA CREER UNE TABLE DE TAILLE (i = nombre de processus) DONT CHAQUE CASE 
  CONTIENT UN TABLEAU DYNAMIQUE DES DIFFERENTS INSTANTS DU BLOQUAGE ET REVEIL
  DE CHAQUE PROCESSUS AVEC ORDRE POUR CONSTRUIRE LES INTERVALLES DANS LESQUELS 
  LE PROCESSUS EST A L'ETAT BLOQUE 
  -----------------------------------------------------------------------------*/
  var TableauFIFO2 = Array(i);
  for (var i1 = 0; i1 < i; i1++) {
    TableauFIFO2[i1] = [];
  }
  /*-----------------------------------------------------------------------------*/
        ...
        if (this.#processeur.getProcessus().getTempsRestant() === 0) {
          ...
          if (
            ...
            if (
              ...
              L'INSTANT DU BLOQUAGE DANS LE TABLEAU ASSOCIE A CE PROCESSUS
              ----------------------------------------------------------------------*/
              TableauFIFO2[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
              /*----------------------------------------------------------------------*/
              console.log(
                "t = " +
                t +
    ...
    if (!arret) {
      for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
        if (
          ...
          /*----------------------------------------------------------------------
          LORSQUE LE PROCESSUS SE REVEIL APRES LE BLOQUAGE SUITE A UNE INTERRUPTION 
          ON ENREGISTRE L'INSTANT DU REVEIL DANS LE TABLEAU ASSOCIE A CE PROCESSUS
          ----------------------------------------------------------------------*/
          TableauFIFO2[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
  ...
  /*---------------------------------------------------------------*/
  return TableauFIFO2; /* ON RETOURNE LE TABLEAU DES INSTANTS */
  /*---------------------------------------------------------------*/
  // console.log("\n--------Fin de l'éxecution---------");
}
/***********************************Ordonnanceur FIFO (3)***************************************************/
Ordonnanceur_FIFO3() /* POUR CONSTRUIRE LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST PRET */ {
  let i = this.#processus.length,
    j = 0,
    t = 0,
    h = 0,
  ...
  /*-----------------------------------------------------------------------------
  ON VA CREER UNE TABLE DE TAILLE (i = nombre de processus) DONT CHAQUE CASE 
  CONTIENT UN TABLEAU DYNAMIQUE DES DIFFERENTS INSTANTS DU CREATION, ACTIVATION, 
  DISACTIVATION ET REVEIL DE CHAQUE PROCESSUS AVEC ORDRE POUR CONSTRUIRE LES 
  INTERVALLES DANS LESQUELS LE PROCESSUS EST A L'ETAT PRET 
  -----------------------------------------------------------------------------*/
  var TableauFIFO3 = Array(i);
  for (var i1 = 0; i1 < i; i1++) {
    TableauFIFO3[i1] = [];
  }
  /*-----------------------------------------------------------------------------*/
  while (j !== i) {
    ...
    while (h < this.#processus.length) {
      if (this.#processus[h].getTempsArrive() === t) {
        /*----------------------------------------------------------------------
        LORSQUE LE PROCESSUS SE CREE ON ENREGISTRE L'INSTANT DE SA CREATION 
        DANS LE TABLEAU ASSOCIE A CE PROCESSUS
        ----------------------------------------------------------------------*/
        TableauFIFO3[this.#processus[h].getPCB().getPID() - 1].push(t);
        /*----------------------------------------------------------------------*/
        console.log(
          "t = " +
          t +
    ...
    if (this.#dispatcher.getSignal() === true) {
      if (this.#files.getFile(0).getFile().length !== 0) {
        ...
        this.#dispatcher.IncrementerNb();
        /*----------------------------------------------------------------------
        LORSQUE LE PROCESSUS S'ACTIVE ON ENREGISTRE L'INSTANT DE SON ACTIVATION 
        DANS LE TABLEAU ASSOCIE A CE PROCESSUS
        ----------------------------------------------------------------------*/
        TableauFIFO3[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
        /*----------------------------------------------------------------------*/
        console.log(
          "t = " +
          t +
    ...
    if (!arret) {
      for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
        if (
          ...
          /*----------------------------------------------------------------------
          LORSQUE LE PROCESSUS SE REVEILLE APRES LE BLOQUAGE SUITE A UNE 
          INTERRUPTION ON ENREGISTRE L'INSTANT DU REVEIL DANS LE TABLEAU
           ASSOCIE A CE PROCESSUS
          ----------------------------------------------------------------------*/
          TableauFIFO3[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
  ...
  /*---------------------------------------------------------------*/
  return TableauFIFO3; /* ON RETOURNE LE TABLEAU DES INSTANTS */
  /*---------------------------------------------------------------*/
  // console.log("\n--------Fin de l'éxecution---------");
}
}
/*-------------------------------------------------------------------
FONCTIONS POUR RELIER LES DONNES DU FORMULAIRE AVEC LES STRUCTURES 
DE DONNEES POUR POUVOIR CREER LES CHARTES ET LE TABLEAU
-------------------------------------------------------------------*/
...
scheduler. Ordonnanceur_FIFO();

/*----------------------------------------------------------------------------------------*/
/* CREER LA TABLE QUI CONTIENT LES TRANSLATIONS DU TEMPS DANS LESQUELLES LE PROCESSUS EST A L'ETAT ACTIF */
...
Init1(scheduler1);
var TableFIFO1 = new Array(scheduler1.getProcessus().length);
TableFIFO1 = scheduler1.Ordonnanceur_FIFO1();
/*----------------------------------------------------------------------------------------*/
/* CREER LA TABLE QUI CONTIENT LES TRANSLATIONS DU TEMPS DANS LESQUELLES LE PROCESSUS EST A L'ETAT BLOQUE */
...
Init1(scheduler2);
var TableFIFO2 = new Array(scheduler2.getProcessus().length);
TableFIFO2 = scheduler2.Ordonnanceur_FIFO2();
/*----------------------------------------------------------------------------------------*/
/* CREER LA TABLE QUI CONTIENT LES TRANSLATIONS DU TEMPS DANS LESQUELLES LE PROCESSUS EST A L'ETAT PRET */
...
Init1(scheduler3);
var TableFIFO3 = new Array(scheduler3.getProcessus().length);
TableFIFO3 = scheduler3.Ordonnanceur_FIFO3();
/*----------------------------------------------------------------------------------------*/


/*--------------------------------------------------------------*/
/* UNE FONCTION POUR REGLER LA DATE SUR LES CHARTES GRAPHIQUES EN SECONDES
/*--------------------------------------------------------------*/
function getDate(sec) {
return (new Date(0, 0, 0, 0, sec / 60, sec % 60));
}
/*--------------------------------------------------------------*/
/* LA CREATION DES CHARTES GRAPHIQUES EN UTILISANT DES BIBLIORTHES SUR GOOLE CHARTS */

google.charts.load("current", { packages: ["timeline"] });
google.charts.setOnLoadCallback(drawChart);
function drawChart() {
var container = document.getElementById("FIFO");
...
var elm = 0;
/* DESSINER LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST ACTIF */
for (var i = 0; i < TableFIFO1.length; i++) {
  elm = 0;
  for (var j = 0; j < TableFIFO1[i].length / 2; j++) {
    dataTable.addRows([
      [
        "P" + (i + 1),
        "Processus Actif",
        getDate(TableFIFO1[i][j + elm]),
        getDate(TableFIFO1[i][j + elm + 1]),
      ]
    ]);
    elm = elm + 1;
  }
}
/* DESSINER LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST BLOQUE */
for (var i = 0; i < TableFIFO2.length; i++) {
  elm = 0;
  for (var j = 0; j < TableFIFO2[i].length / 2; j++) {
    dataTable.addRows([
      [
        "P" + (i + 1),
        "Processus Bloque",
        getDate(TableFIFO2[i][j + elm]),
        getDate(TableFIFO2[i][j + elm + 1]),
      ]
    ]);
    elm = elm + 1;
  }
}
/* DESSINER LES INTERVALLES DU TEMPS LORSQUE LE PROCESSUS EST PRET */
for (var i = 0; i < TableFIFO3.length; i++) {
  elm = 0;
  for (var j = 0; j < TableFIFO3[i].length / 2; j++) {
    dataTable.addRows([
      [
        "P" + (i + 1),
        "Processus Pret",
        getDate(TableFIFO3[i][j + elm]),
        getDate(TableFIFO3[i][j + elm + 1]),
      ]
    ]);
    elm = elm + 1;
  }
...
/*--------------------------------------------------------------*/
/* POUR ASSURER LA RESPONSIVITE DE CHARTES */
/*--------------------------------------------------------------*/
