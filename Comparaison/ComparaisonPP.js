...
class Scheduler {
  ...
  /*******Ordonnanceur de priorité preemptif utilisé pour la creation des tables********** */
  Ordonnanceur_PRIOS_Preemptif()
{
  let i = this.#processus.length , j = 0 , t = 0 , h = 0 , quantum = 0 , arret = false , cpt = 0 , num_file = 0 ;
  let somme_res=0,somme_at=0 ,somme_rep=0,nbr_at=0,nbr_rep=0,nbr_res=0,taux=0,ut=0;
  while(j !== i){
  ...
  Ordonnanceur_PRIOS_Preemptif1() { /* ACTIF */
    let i = this.#processus.length,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPP1 = new Array(i);
    for (var i1 = 0; i1 < TableauPP1.length; i1++) {
      TableauPP1[i1] = [];
    }
        ...
        if (this.#processus[h].getTempsArrive() === t) {
          ...
          if (this.#processeur.ProcesseurActif()) {
            if (
              ...
              /*----------------------------------------------------------------------*/
              TableauPP1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
              /*----------------------------------------------------------------------*/
              console.log(
                "t = " +
                t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            TableauPP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
  ...
  Ordonnanceur_PRIOS_Preemptif3() { /* PRET */
    let i = this.#processus.length,
      j = 0,
      t = 0,
      h = 0,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPP3 = new Array(i);
    for (var i1 = 0; i1 < TableauPP3.length; i1++) {
      TableauPP3[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
      ...
      while (h < this.#processus.length) {
        if (this.#processus[h].getTempsArrive() === t) {
          /*----------------------------------------------------------------------*/
          TableauPP3[this.#processus[h].getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
          ...
          if (this.#processeur.ProcesseurActif()) {
            if (
              ...
              /*----------------------------------------------------------------------*/
              TableauPP3[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
              /*----------------------------------------------------------------------*/
              console.log(
                "t = " +
                t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPP3[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
              "t = " +
              t +
  ...
  Ordonnanceur_PRIOS_Preemptif2() { /* BLOQUE */
    let i = this.#processus.length,
      j = 0,
      t = 0,
      h = 0,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPP2 = new Array(i);
    for (var i1 = 0; i1 < TableauPP2.length; i1++) {
      TableauPP2[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
            ...
            if (
              ...
              if (
                ...
                if (
                  ...
                  /*----------------------------------------------------------------------*/
                  TableauPP2[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                  /*----------------------------------------------------------------------*/
                  console.log(
                    "t = " +
                    t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPP2[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
              "t = " +
              t +
...
Init1(scheduler1);
var TablePP1 = new Array(scheduler1.getProcessus().length);
TablePP1 = scheduler1.Ordonnanceur_PRIOS_Preemptif1();
...
Init1(scheduler2);
var TablePP2 = new Array(scheduler2.getProcessus().length);
TablePP2 = scheduler2.Ordonnanceur_PRIOS_Preemptif2();
...
Init1(scheduler3);
var TablePP3 = new Array(scheduler3.getProcessus().length);
TablePP3 = scheduler3.Ordonnanceur_PRIOS_Preemptif3();
...
function drawChart() {
  ;
  var container = document.getElementById("PP");
  ...
  var elm = 0;
  for (var i = 0; i < TablePP1.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePP1[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Actif",
          getDate(TablePP1[i][j + elm]),
          getDate(TablePP1[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
  }
  for (var i = 0; i < TablePP2.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePP2[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Bloque",
          getDate(TablePP2[i][j + elm]),
          getDate(TablePP2[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
  }
  for (var i = 0; i < TablePP3.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePP3[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Pret",
          getDate(TablePP3[i][j + elm]),
          getDate(TablePP3[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
