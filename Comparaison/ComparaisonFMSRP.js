...
class Processeur {
  ...
  class Scheduler {
    ...
    /************************Ordonnanceur files multiniveaux avec recyclage utilisé pour la creation des tables************ */
    Ordonnanceur_FMSR_Preemptif() {
        let i = this.#processus.length , j = 0 , t = 0 , h = 0 , quantum , somme_at=0,somme_rep=0,nbr_at=0,taux,ut,nbr_rep=0,nbr_res=0,somme_res=0, arret= false , cpt = 0 , num_file = 0 ;
        while (j !== i) {
            ...
            if (!arret) {
                for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                    if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                        ...
                        if (this.#processeur.ProcesseurActif()) {
                            if ((this.#fileBloquee.getFile()[j].getPriorite() % 3) < (this.#processeur.getProcessus().getPriorite() % 3)) {
                                console.log('t = ' + t + ' : ' + 'Désactivation du processus' + this.#processeur.getProcessus().getPCB().getPID());
  ...
  /*----------------------------------------------------------------------------------------------------------*/
  Ordonnanceur_FMSR_Preemptif1() { /* ACTIVER */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRP1 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRP1.length; i1++) {
      TableauFMSRP1[i1] = [];
    }
    ...
    while (j !== i) {
        ...
        while (h < this.#processus.length) {
            if (this.#processus[h].getTempsArrive() === t) {
                ...
                if (this.#processeur.ProcesseurActif()) {
                    if ((this.#files.getFile(0).getFile()[this.#files.getFile(0).getFile().length - 1].getPriorite() % 3) < (this.#processeur.getProcessus().getPriorite() % 3)) {
                        /*----------------------------------------------------------------------*/
                        TableauFMSRP1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                         /*----------------------------------------------------------------------*/
                        console.log('t = ' + t + ' : ' + 'Désactivation du processus' + this.#processeur.getProcessus().getPCB().getPID());
                        this.DesactiverProcessus(this.#processeur.getProcessus().getPriorite() % 3);
                        this.#dispatcher.setSignal(true);
        ...
        if (this.#dispatcher.getSignal() === true) {
            ...
            if (num_file !== -1) {
                ...
                if(num_file == 0){
                    quantum = this.getFiles().getFile(num_file).getQuantum();
                }else{
                    quantum = undefined ;
                }
                console.log(quantum) ;
                this.getDispatcher().setSignal(false);
            } else {
                console.log('t = ' + t + ' : ' + 'Aucun processus à activer !');
            }
        } else { //Si le processeur est actif
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    TableauFMSRP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
                    this.#fileBloquee.getFile()[j].DetruireInterruption(0);
                    let num1 = this.#fileBloquee.getFile()[j].getPriorite() ;
                    this.ReveillerProcessus(j, this.#fileBloquee.getFile()[j].getPriorite());
                    if (this.#processeur.ProcesseurActif()) {
                        if ((num1 % 3) < (this.#processeur.getProcessus().getPriorite() % 3)) {
                            /*----------------------------------------------------------------------*/
                            TableauFMSRP1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                            /*----------------------------------------------------------------------*/
                            console.log('t = ' + t + ' : ' + 'Désactivation du processus' + this.#processeur.getProcessus().getPCB().getPID());
    ...
    console.log('Nombre de changements de contexte : ' + this.#dispatcher.getNbChangementContexte());
    console.log("\n--------Fin de l'éxecution---------");
    return TableauFMSRP1;
}
/*--------------------------------------------------------------------------------------------------------------*/
Ordonnanceur_FMSR_Preemptif2() { /* BLOQUER */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRP2 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRP2.length; i1++) {
      TableauFMSRP2[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
                    ...
                    if (cpt === quantum) {
                        ...
                        if (this.#processeur.getProcessus().getInterruptions().length > 0) {
                            if (this.#processeur.getProcessus().getInterruptions()[0].getTempsDeclenchement() === this.#processeur.getProcessus().getTempsExecution() - this.#processeur.getProcessus().getTempsRestant()) {
                                this.#processeur.getProcessus().setPriorite(num_file);
                                /*----------------------------------------------------------------------*/
                                TableauFMSRP2[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                                 /*----------------------------------------------------------------------*/
                                console.log('t = ' + t + ' : ' + 'Bloquage du processus' + this.#processeur.getProcessus().getPCB().getPID() + ': Interruption');
                                this.BloquerProcessus();
                                cpt = 0;
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRP2[this.#fileBloquee.getFile()[j].getPCB().getPID()- 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
                    this.#fileBloquee.getFile()[j].DetruireInterruption(0);
                    let num1 =  this.#fileBloquee.getFile()[j].getPriorite() % 3 ;
                    this.ReveillerProcessus(j, this.#fileBloquee.getFile()[j].getPriorite() % 3);
                    if (this.#processeur.ProcesseurActif()) {
    ...
    console.log('Nombre de changements de contexte : ' + this.#dispatcher.getNbChangementContexte());
    console.log("\n--------Fin de l'éxecution---------");
    return TableauFMSRP2;
}
/*----------------------------------------------------------------------------------------------------------*/
Ordonnanceur_FMSR_Preemptif3() { /* PRET */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRP3 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRP3.length; i1++) {
      TableauFMSRP3[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
        ...
        while (h < this.#processus.length) {
            if (this.#processus[h].getTempsArrive() === t) {
                /*----------------------------------------------------------------------*/
                TableauFMSRP3[this.#processus[h].getPCB().getPID()- 1].push(t);
                /*----------------------------------------------------------------------*/
                console.log('t = ' + t + ' : ' + 'Création du processus' + this.#processus[h].getPCB().getPID());
                this.CreerProcessus(h, 0);
                if (this.#processeur.ProcesseurActif()) {
                    if ((this.#files.getFile(0).getFile()[this.#files.getFile(0).getFile().length - 1].getPriorite() % 3) < (this.#processeur.getProcessus().getPriorite() % 3)) {
                        /*----------------------------------------------------------------------*/
                        TableauFMSRP3[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                        /*----------------------------------------------------------------------*/
                        console.log('t = ' + t + ' : ' + 'Désactivation du processus' + this.#processeur.getProcessus().getPCB().getPID());
                        this.DesactiverProcessus(this.#processeur.getProcessus().getPriorite() % 3);
                        this.#dispatcher.setSignal(true);
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRP3[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
                    this.#fileBloquee.getFile()[j].DetruireInterruption(0);
                    let num1 = this.#fileBloquee.getFile()[j].getPriorite() % 3 ;
...
scheduler. Ordonnanceur_FMSR_Preemptif();
  ...
  Init_FMSR(scheduler1);
  var TableFMSRP1 = new Array(scheduler1.getProcessus().length);
  TableFMSRP1 = scheduler1.Ordonnanceur_FMSR_Preemptif1();
  ...
  Init_FMSR(scheduler2);
  var TableFMSRP2 = new Array(scheduler2.getProcessus().length);
  TableFMSRP2 = scheduler2.Ordonnanceur_FMSR_Preemptif2();
  ...
  Init_FMSR(scheduler3);
  var TableFMSRP3 = new Array(scheduler3.getProcessus().length);
  TableFMSRP3 = scheduler3.Ordonnanceur_FMSR_Preemptif3();
  ...
  function drawChart() {
    ;
    var container = document.getElementById("FMSRP");
    ...
    var elm = 0;
    for (var i = 0; i < TableFMSRP1.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRP1[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Actif",
            getDate(TableFMSRP1[i][j + elm]),
            getDate(TableFMSRP1[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
    }
    for (var i = 0; i < TableFMSRP2.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRP2[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Bloque",
            getDate(TableFMSRP2[i][j + elm]),
            getDate(TableFMSRP2[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
    }
    for (var i = 0; i < TableFMSRP3.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRP3[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Pret",
            getDate(TableFMSRP3[i][j + elm]),
            getDate(TableFMSRP3[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
