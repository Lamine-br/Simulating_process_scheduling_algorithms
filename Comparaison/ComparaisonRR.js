...
class Scheduler {
  ...
  DetruireProcessus(temps) {
      ...
      /*****************Ordonnanceur round robbin utilisé pour la creation des tables******* */
      Ordonnanceur_RR(){
        let i = this.#processus.length , j = 0 , t = 0 , h = 0 , quantum = 0 ,somme_at=0,taux,ut,somme_rep=0,nbr_rep=0,nbr_at=0,nbr_res=0,somme_res=0, arret = false , cpt = 0 , num_file = 0 ;
        while(j !== i){
        ...
        /*************************Ordonnanceur Round Robin (1)*************************/
        Ordonnanceur_RR1() { /* ACTIF */
            ...
            num_file = 0;
          /*--------------------------------------------------------------------------------*/
            var TableauRR1 = new Array(i);
            for(var i1=0; i1<TableauRR1.length;i1++)
            {
              TableauRR1[i1]= [];
            }
          /*--------------------------------------------------------------------------------*/
            while (j !== i) {
              ...
              if (this.#dispatcher.getSignal() === true) {
                if (this.#files.getFile(0).getFile().length !== 0) {
                 ...
                 /*----------------------------------------------------------------------*/
                  TableauRR1[this.#processeur.getProcessus().getPCB().getPID()-1].push(t);
                 /*----------------------------------------------------------------------*/
                  console.log(
                    "t = " +
                      t +
              ...
              if (!arret) {
                for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                  if (
                    ...
                    /*----------------------------------------------------------------------*/
                    TableauRR1[this.#fileBloquee.getFile()[j].getPCB().getPID()-1].push(t);
                    /*----------------------------------------------------------------------*/
                    TableauRR1[this.#fileBloquee.getFile()[j].getPCB().getPID()-1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log(
          ...
          /*************************Ordonnanceur Round Robin (2)*************************/
        Ordonnanceur_RR2() { /* BLOQUE */
          let i = this.#processus.length,
          j = 0,
          t = 0,
          h = 0,
          ...
          num_file = 0;
        /*--------------------------------------------------------------------------------*/
          var TableauRR2 = new Array(i);
          for(var i1=0; i1<TableauRR2.length;i1++)
          {
            TableauRR2[i1]= [];
          }
        /*--------------------------------------------------------------------------------*/
                  ...
                  if (cpt === quantum) {
                    ...
                    if (
                      ...
                      if (
                        ...
                        /*----------------------------------------------------------------------*/
                          TableauRR2[this.#processeur.getProcessus().getPCB().getPID()-1].push(t);
                        /*----------------------------------------------------------------------*/
                        console.log(
                          "t = " +
                            t +
            ...
            if (!arret) {
              for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (
                  ...
                  /*----------------------------------------------------------------------*/
                TableauRR2[this.#fileBloquee.getFile()[j].getPCB().getPID()-1].push(t);
                /*----------------------------------------------------------------------*/
                  console.log(
                    "t = " +
                      t +
        ...
        /*************************Ordonnanceur Round Robin (3)*************************/
        Ordonnanceur_RR3() { /* PRET */
          let i = this.#processus.length,
          j = 0,
          t = 0,
          h = 0,
          ...
          num_file = 0;
        /*--------------------------------------------------------------------------------*/
          var TableauRR3 = new Array(i);
          for(var i1=0; i1<TableauRR3.length;i1++)
          {
            TableauRR3[i1]= [];
          }
        /*--------------------------------------------------------------------------------*/
          while (j !== i) {
            ...
            while (h < this.#processus.length) {
              if (this.#processus[h].getTempsArrive() === t) {
                /*----------------------------------------------------------------------*/
                TableauRR3[this.#processus[h].getPCB().getPID()-1].push(t);
               /*----------------------------------------------------------------------*/
                console.log(
                  "t = " +
                    t +
            ...
            if (this.#dispatcher.getSignal() === true) {
              if (this.#files.getFile(0).getFile().length !== 0) {
               ...
               /*----------------------------------------------------------------------*/
                TableauRR3[this.#processeur.getProcessus().getPCB().getPID()-1].push(t);
               /*----------------------------------------------------------------------*/
                console.log(
                  "t = " +
                    t +
            ...
            if (!arret) {
              for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (
                  ...
                  /*----------------------------------------------------------------------*/
                TableauRR3[this.#fileBloquee.getFile()[j].getPCB().getPID()-1].push(t);
                /*----------------------------------------------------------------------*/
                  console.log(
                    "t = " +
                      t +
...
Init1(scheduler1) ;
var TableRR1 = new Array(scheduler1.getProcessus().length);
TableRR1 = scheduler1.Ordonnanceur_RR1();
...
Init1(scheduler2) ;
var TableRR2 = new Array(scheduler2.getProcessus().length);
TableRR2 = scheduler2.Ordonnanceur_RR2();
...
Init1(scheduler3) ;
var TableRR3 = new Array(scheduler3.getProcessus().length);
TableRR3 = scheduler3.Ordonnanceur_RR3();
...
/*--------------------------------------------------------------*/
      ...
      function drawChart() {;
        ...
        var elm=0;
        for(var i=0; i<TableRR1.length; i++){
        elm=0;
            for(var j=0;j<TableRR1[i].length / 2; j++)
            {
                dataTable.addRows([
                    [
                      "P" + (i+1),
                      "Processus Actif",
                      getDate(TableRR1[i][j+elm]),
                      getDate(TableRR1[i][j+elm+1]),
                    ]
                ]);
                elm = elm +1;
            }
          }
          for(var i=0; i<TableRR2.length; i++){
            elm=0;
                for(var j=0;j<TableRR2[i].length / 2; j++)
                {
                    dataTable.addRows([
                        [
                          "P" + (i+1),
                          "Processus Bloque",
                          getDate(TableRR2[i][j+elm]),
                          getDate(TableRR2[i][j+elm+1]),
                        ]
                    ]);
                    elm = elm +1;
                }
              }
              for(var i=0; i<TableRR3.length; i++){
                elm=0;
                    for(var j=0;j<TableRR3[i].length / 2; j++)
                    {
                        dataTable.addRows([
                            [
                              "P" + (i+1),
                              "Processus Pret",
                              getDate(TableRR3[i][j+elm]),
                              getDate(TableRR3[i][j+elm+1]),
                            ]
                        ]);
                        elm = elm +1;
                    }
