...
class Scheduler {
  ...
  /***********Ordonnanceur prios non priemptif utilisé pour la creation des tables***** */
  Ordonnanceur_PRIOS_NonPreemptif()
  {
      let i = this.#processus.length , j = 0 , t = 0 , h = 0 , quantum = 0 ,somme_res=0,somme_at=0,somme_rep=0,nbr_rep=0,nbr_at=0,nbr_res=0,taux,ut, arret = false , cpt = 0 , num_file = 0 ;
  ...
  /*****************Ordonnanceur Par Priorite Statique (1)*****************/
  Ordonnanceur_PRIOS_NonPreemptif1() { /* ACTIF */
    let i = this.#processus.length,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPNP1 = new Array(i);
    for (var i1 = 0; i1 < TableauPNP1.length; i1++) {
      TableauPNP1[i1] = [];
    }
    ...
    while (j !== i) {
      ...
      if (this.#dispatcher.getSignal() === true) {
        ...
        if (num_file !== -1) {
          ...
          this.#dispatcher.IncrementerNb();
          /*----------------------------------------------------------------------*/
          TableauPNP1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPNP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            TableauPNP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
  ...
  /*****************Ordonnanceur Par Priorite Statique (2)*****************/
  Ordonnanceur_PRIOS_NonPreemptif2() {
    let i = this.#processus.length,
      j = 0,
      t = 0,
      h = 0,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPNP2 = new Array(i);
    for (var i1 = 0; i1 < TableauPNP2.length; i1++) {
      TableauPNP2[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
            ...
            if (
              ...
              if (
                ...
                if (
                  ...
                  /*----------------------------------------------------------------------*/
                  TableauPNP2[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                  /*----------------------------------------------------------------------*/
                  console.log(
                    "t = " +
                    t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPNP2[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
              "t = " +
              t +
  ...
  /*****************Ordonnanceur Par Priorite Statique (3)*****************/
  Ordonnanceur_PRIOS_NonPreemptif3() {
    let i = this.#processus.length,
      j = 0,
      t = 0,
      h = 0,
      ...
      num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauPNP3 = new Array(i);
    for (var i1 = 0; i1 < TableauPNP3.length; i1++) {
      TableauPNP3[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
      ...
      while (h < this.#processus.length) {
        if (this.#processus[h].getTempsArrive() === t) {
          /*----------------------------------------------------------------------*/
          TableauPNP3[this.#processus[h].getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
      ...
      if (this.#dispatcher.getSignal() === true) {
        ...
        if (num_file !== -1) {
          ...
          this.#dispatcher.IncrementerNb();
          /*----------------------------------------------------------------------*/
          TableauPNP3[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
          /*----------------------------------------------------------------------*/
          console.log(
            "t = " +
            t +
      ...
      if (!arret) {
        for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
          if (
            ...
            /*----------------------------------------------------------------------*/
            TableauPNP3[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
            /*----------------------------------------------------------------------*/
            console.log(
              "t = " +
              t +
...
scheduler. Ordonnanceur_PRIOS_NonPreemptif();
...
Init1(scheduler1);
var TablePNP1 = new Array(scheduler1.getProcessus().length);
TablePNP1 = scheduler1.Ordonnanceur_PRIOS_NonPreemptif1();
...
Init1(scheduler2);
var TablePNP2 = new Array(scheduler2.getProcessus().length);
TablePNP2 = scheduler2.Ordonnanceur_PRIOS_NonPreemptif2();
...
Init1(scheduler3);
var TablePNP3 = new Array(scheduler3.getProcessus().length);
TablePNP3 = scheduler3.Ordonnanceur_PRIOS_NonPreemptif3();
...
function drawChart() {
  ;
  var container = document.getElementById("PNP");
  ...
  var elm = 0;
  for (var i = 0; i < TablePNP1.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePNP1[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Actif",
          getDate(TablePNP1[i][j + elm]),
          getDate(TablePNP1[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
  }
  for (var i = 0; i < TablePNP2.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePNP2[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Bloque",
          getDate(TablePNP2[i][j + elm]),
          getDate(TablePNP2[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
  }
  for (var i = 0; i < TablePNP3.length; i++) {
    elm = 0;
    for (var j = 0; j < TablePNP3[i].length / 2; j++) {
      dataTable.addRows([
        [
          "P" + (i + 1),
          "Processus Pret",
          getDate(TablePNP3[i][j + elm]),
          getDate(TablePNP3[i][j + elm + 1]),
        ]
      ]);
      elm = elm + 1;
    }
