...
class Scheduler {
  ...
  Ordonnanceur_FMAR(){
    let i = this.#processus.length , j = 0 , t = 0 , h = 0 ,somme_res=0,somme_at=0,somme_rep=0,nbr_at=0,nbr_rep=0,taux,ut,nbr_res=0, quantum = 0 , arret = false , cpt = 0 , num_file = 0 ;
    while(j !== i){
  ...
  /*----------------------------------------------------------------------------------------------------------*/
  Ordonnanceur_FMSR_NonPreemptif1() { /* ACTIVER */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRNP1 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRNP1.length; i1++) {
        TableauFMSRNP1[i1] = [];
    }
    ...
    while (j !== i) {
        ...
        if (this.#dispatcher.getSignal() === true) {
            ...
            if (num_file !== -1) {
                ...
                this.#dispatcher.IncrementerNb();
                /*----------------------------------------------------------------------*/
                TableauFMSRNP1[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                /*----------------------------------------------------------------------*/
                console.log('t = ' + t + ' : ' + 'Activation du processus' + this.#processeur.getProcessus().getPCB().getPID());
                if (this.#processeur.getProcessus().getTempsExecution() === this.#processeur.getProcessus().getTempsRestant()) {
                    this.#processeur.getProcessus().setTempsReponse(this.#processeur.getProcessus().getTempsAttente());
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRNP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    TableauFMSRNP1[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
    ...
    console.log('Nombre de changements de contexte : ' + this.#dispatcher.getNbChangementContexte());
    console.log("\n--------Fin de l'éxecution---------");
    return TableauFMSRNP1;
}
/*----------------------------------------------------------------------------------------------------------*/
Ordonnanceur_FMSR_NonPreemptif2() { /* BLOQUER */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRNP2 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRNP2.length; i1++) {
        TableauFMSRNP2[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
                    ...
                    if (cpt === quantum) {
                        ...
                        if (this.#processeur.getProcessus().getInterruptions().length > 0) {
                            if (this.#processeur.getProcessus().getInterruptions()[0].getTempsDeclenchement() === this.#processeur.getProcessus().getTempsExecution() - this.#processeur.getProcessus().getTempsRestant()) {
                                this.#processeur.getProcessus().setPriorite(num_file);
                                /*----------------------------------------------------------------------*/
                                TableauFMSRNP2[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                                /*----------------------------------------------------------------------*/
                                console.log('t = ' + t + ' : ' + 'Bloquage du processus' + this.#processeur.getProcessus().getPCB().getPID() + ': Interruption');
                                this.BloquerProcessus();
                                cpt = 0;
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRNP2[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
                    this.#fileBloquee.getFile()[j].DetruireInterruption(0);
                    this.ReveillerProcessus(j, this.#fileBloquee.getFile()[j].getPriorite());
    ...
    console.log('Nombre de changements de contexte : ' + this.#dispatcher.getNbChangementContexte());
    console.log("\n--------Fin de l'éxecution---------");
    return TableauFMSRNP2;
}
/*----------------------------------------------------------------------------------------------------------*/
Ordonnanceur_FMSR_NonPreemptif3() { /* PRET */
    let i = this.#processus.length, j = 0, t = 0, h = 0, quantum = 0, arret = false, cpt = 0, num_file = 0;
    /*--------------------------------------------------------------------------------*/
    var TableauFMSRNP3 = new Array(i);
    for (var i1 = 0; i1 < TableauFMSRNP3.length; i1++) {
        TableauFMSRNP3[i1] = [];
    }
    var i2 = 0; var i3 = 0; var i4 = 0; var i5 = 0;
    /*--------------------------------------------------------------------------------*/
    while (j !== i) {
        ...
        while (h < this.#processus.length) {
            if (this.#processus[h].getTempsArrive() === t) {
                /*----------------------------------------------------------------------*/
                TableauFMSRNP3[this.#processus[h].getPCB().getPID() - 1].push(t);
                /*----------------------------------------------------------------------*/
                console.log('t = ' + t + ' : ' + 'Création du processus' + this.#processus[h].getPCB().getPID());
                this.CreerProcessus(h, 0);
            } else {
        ...
        if (this.#dispatcher.getSignal() === true) {
            ...
            if (num_file !== -1) {
                ...
                this.#dispatcher.IncrementerNb();
                /*----------------------------------------------------------------------*/
                TableauFMSRNP3[this.#processeur.getProcessus().getPCB().getPID() - 1].push(t);
                /*----------------------------------------------------------------------*/
                console.log('t = ' + t + ' : ' + 'Activation du processus' + this.#processeur.getProcessus().getPCB().getPID());
                if (this.#processeur.getProcessus().getTempsExecution() === this.#processeur.getProcessus().getTempsRestant()) {
                    this.#processeur.getProcessus().setTempsReponse(this.#processeur.getProcessus().getTempsAttente());
        ...
        if (!arret) {
            for (let j = 0; j < this.#fileBloquee.getFile().length; j++) {
                if (this.#fileBloquee.getFile()[j].getInterruptions()[0].getTempsBlocage() === 0) {
                    /*----------------------------------------------------------------------*/
                    TableauFMSRNP3[this.#fileBloquee.getFile()[j].getPCB().getPID() - 1].push(t);
                    /*----------------------------------------------------------------------*/
                    console.log('t = ' + (t) + ' : ' + 'Réveil du processus' + this.#fileBloquee.getFile()[j].getPCB().getPID());
                    this.#fileBloquee.getFile()[j].DetruireInterruption(0);
                    this.ReveillerProcessus(j, this.#fileBloquee.getFile()[j].getPriorite());
  ...
  TableFMSRNP1 = scheduler1.Ordonnanceur_FMSR_NonPreemptif1();
  ...
  TableFMSRNP2 = scheduler2.Ordonnanceur_FMSR_NonPreemptif2();
  ...
  TableFMSRNP3 = scheduler3.Ordonnanceur_FMSR_NonPreemptif3();
  ...
  function drawChart() {
    ;
    var container = document.getElementById("FMSRNP");
    ...
    var elm = 0;
    for (var i = 0; i < TableFMSRNP1.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRNP1[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Actif",
            getDate(TableFMSRNP1[i][j + elm]),
            getDate(TableFMSRNP1[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
    }
    for (var i = 0; i < TableFMSRNP2.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRNP2[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Bloque",
            getDate(TableFMSRNP2[i][j + elm]),
            getDate(TableFMSRNP2[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
    }
    for (var i = 0; i < TableFMSRNP3.length; i++) {
      elm = 0;
      for (var j = 0; j < TableFMSRNP3[i].length / 2; j++) {
        dataTable.addRows([
          [
            "P" + (i + 1),
            "Processus Pret",
            getDate(TableFMSRNP3[i][j + elm]),
            getDate(TableFMSRNP3[i][j + elm + 1]),
          ]
        ]);
        elm = elm + 1;
      }
